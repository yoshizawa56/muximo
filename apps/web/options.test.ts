...
type WebInput = { environment?: NodeJS.ProcessEnv };
...
const cases = [
  {
    name: "uses Web defaults without selecting a daemon instance",
    input: {},
    assert: [
  ...
  {
    name: "applies Web-specific values and ignores muximod-only values",
    input: {
      environment: {
        HOME: "/home/test",
...
const table: OperationTable<undefined, "default", WebInput, WebResult, WebContext> = {
  ...
  cases,
  execute: (_fixture, input) => resolveWebOptions({ HOME: "/home/test", ...input.environment }, "/workspace"),
