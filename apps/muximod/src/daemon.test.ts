...
const cases = [
  {
    ...
    assert: [
      returns<Context, Result>([
        ...
        "49819",
        "--pid-file",
        "/private/tmp/muximo-daemon-test.pid",
        "--control-socket",
        "/private/tmp/muximo-daemon-test.sock",
        "--muximod-base-url",
        "http://127.0.0.1:49819",
        "--log-level",
        "debug",
        "--log-file",
        "/private/tmp/muximo-daemon-test.log",
      ]),
    ],
  },
...
const healthLogCases = [
  {
    ...
    input: {
      records: [
        JSON.stringify({
          ...
          level: "warn",
          pid: 123,
          event: "tmux.default_session_failed",
          fields: { error: { message: "tmux unavailable" }, errorId: "warn-1" },
        }),
        JSON.stringify({
          ...
          fields: {
            error: { message: "Failed to start server. Is port 45431 in use?", code: "EADDRINUSE" },
            errorId: "error-1",
          },
        }),
        JSON.stringify({
    ...
    assert: [
      ...
      containsHealthOutput(
        "ERROR daemon.start_failed: Failed to start server. Is port 45431 in use? code=EADDRINUSE errorId=error-1",
      ),
    ],
  },
  {
    ...
    input: {
      records: [
        JSON.stringify({
          ...
          fields: { host: "127.0.0.1", port: 4317 },
        }),
        "not-json",
      ],
      startedAt: Date.parse("2026-08-21T00:00:01.000Z"),
    ...
    input: {
      records: [
        JSON.stringify({
          ...
          pid: 123,
          event: "daemon.start_failed",
          fields: { error: { message: "old failure", code: "EADDRINUSE" }, errorId: "old" },
        }),
        JSON.stringify({
          timestamp: "2026-08-21T00:00:03.000Z",
          level: "error",
          pid: 456,
          event: "daemon.start_failed",
          fields: { error: { message: "other process failure", code: "EADDRINUSE" }, errorId: "other" },
        }),
        JSON.stringify({
          timestamp: "2026-08-21T00:00:04.000Z",
          level: "error",
          pid: 123,
          event: "daemon.start_failed",
          fields: { error: { message: "current failure", code: "EADDRINUSE" }, errorId: "current" },
        }),
      ],
...
const createHealthLogFixture = (): FixtureHandle<HealthLogFixture> => {
  ...
  return {
    fixture: {
      ...
      prepare: (records) =>
        writeFileSync(join(root, "muximod.log"), records.length > 0 ? `${records.join("\n")}\n` : ""),
    },
