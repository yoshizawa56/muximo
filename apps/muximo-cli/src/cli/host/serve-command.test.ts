...
const parseCases = [
  {
    ...
    input: {
      args: ["tailscale"],
      environment: { MUXIMOD_PORT: "4391", MUXIMO_SERVE_PORT: "8444", TAILSCALE_BIN: "tailscale-test" },
    },
    assert: [
      matchesOptions({
        provider: "tailscale",
  ...
  },
  {
    ...
    input: {
      args: ["tailscale", "--log-level", "debug", "--log-file", "/private/tmp/muximo-serve.log"],
      environment: {},
    },
...
type RunContext = {
  ensured: readonly unknown[];
  calls: readonly { command: string; args: string[] }[];
  summary: string;
  error: string;
  helpBackground: boolean;
  helpBgFlag: boolean;
};
...
const runCases = [
  {
    ...
    input: {
      args: ["tailscale", "--port", "443"],
      environment: { MUXIMOD_PORT: "4391", MUXIMO_TAILSCALE_HOSTNAME: "muximo-host.tailnet.ts.net" },
    },
    assert: [
      ...
      hasObserved<RunContext, number>("calls", [
        { command: "tailscale", args: ["serve", "--bg", "--https=443", "--yes", "http://127.0.0.1:4391"] },
      ]),
      hasObserved<RunContext, number>(
        "summary",
        "muximo serve tailscale: https://muximo-host.tailnet.ts.net/ -> http://127.0.0.1:4391",
      ),
    ],
  },
  {
    ...
    assert: [
      returns<RunContext, number>(0),
      hasObserved<RunContext, number>("ensured", []),
      hasObserved<RunContext, number>("calls", []),
      hasObserved<RunContext, number>("helpBackground", true),
      hasObserved<RunContext, number>("helpBgFlag", true),
    ],
  },
