...
export const createSession = Effect.fn("Sessions.create")(function* (input: { name: string; workspaceId: string }) {
  ...
  const workspace = yield* workspaceCatalog.resolveWorkspaceDirectory(
    yield* attemptSync(() => WorkspaceId.create(input.workspaceId)),
  ...
  return yield* Effect.acquireUseRelease(
    ...
    (managedSessionId) =>
      Effect.gen(function* () {
        ...
        if (initialPane) {
          shellPane = yield* attemptSync(() => {
            const updated = initialPane.update({
              kind: "shell",
              agentId: clearPatch,
            });
            return updated.state !== "running"
              ? updated.transitionTo("running", "session created", clock.now())
              : updated;
          });
