...
export class RunAgentSession {
  ...
  public readonly prepare = Effect.fn("AgentSessions.prepare")(
    ...
    function* (this: RunAgentSession, input: StartAgentSessionInput, signal?: AbortSignal) {
      ...
      if (existing && !(yield* self.recoverAbandonedExecution(existing, signal))) {
        return yield* Effect.fail(new Error(`session name already exists in this workspace: ${name}`));
      }
      yield* checkAborted(signal);
      ...
      const body = Effect.gen(function* () {
        ...
        if (!(yield* deps.worktrees.copyFiles(session))) {
          ...
          return yield* Effect.fail(new Error("worktree file copy failed"));
        }
        yield* checkAborted(signal);
        ...
        if (!setup.success) {
          yield* self.markSetupFailed(session);
          return yield* Effect.fail(new Error("setup hook failed"));
        }
        ...
        if (!baseline.success) {
          yield* self.markSetupFailed(session);
          return yield* Effect.fail(new Error("backend rollout baseline capture failed"));
        }
  ...
  private readonly recoverAbandonedExecution = Effect.fn("AgentSessions.recoverAbandoned")(
    ...
    function* (this: RunAgentSession, session: AgentSession, signal?: AbortSignal) {
      ...
      if (this.preparing.has(session.id)) {
        return yield* Effect.fail(new Error(`session '${session.name}' is still being prepared`));
      }
      if (this.recovering.has(session.id)) {
        return yield* Effect.fail(new Error(`session '${session.name}' is already being recovered`));
      }
      if (this.completions.has(session.executionId)) {
        return yield* Effect.fail(new Error(`session '${session.name}' is being finalized`));
      }
      ...
      const body = Effect.gen(function* () {
        const recoveringSession = session.update({
          status: "recovering",
          resuming: false,
        ...
        yield* deps.launcher.disposeLaunch(recoveringSession).pipe(
          Effect.catch((error) => {
            ...
            return Effect.fail(
              new Error(`abandoned session '${recoveringSession.name}' could not release its backend resources`, {
                cause: error,
              }),
            );
          }),
        ...
        if (cleanup.disposition !== "removed") {
          return yield* Effect.fail(
            new Error(`abandoned session '${failed.name}' could not be cleaned up: ${cleanup.reason}`),
          );
        }
        return true;
      });
  ...
  private readonly completeOnce = Effect.fn("AgentSessions.completeRunOnce")(
    ...
    function* (this: RunAgentSession, input: CompleteAgentSessionInput) {
      ...
      if (this.recoveringExecutions.has(input.executionId)) {
        return yield* Effect.fail(new Error(`agent execution '${input.executionId}' is being recovered`));
      }
      ...
      let session = yield* deps.sessions
        .findById(AgentSessionId.create(input.agentSessionId))
        .pipe(
          Effect.flatMap((found) =>
            found ? Effect.succeed(found) : Effect.fail(new Error(`agent session not found: ${input.agentSessionId}`)),
          ),
        );
      if (session.executionId !== input.executionId)
        return yield* Effect.fail(new Error("agent execution is no longer current"));
      if (session.status !== "running")
        return yield* Effect.fail(new Error(`agent session '${session.name}' is not running`));
  ...
  private fromReceipt(
    ...
    if (
      ...
      return Effect.fail(new Error("agent execution receipt does not match the completion request"));
    }
