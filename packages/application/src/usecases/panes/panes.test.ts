...
class FakeGateway implements PaneGateway {
  ...
  public async sendInput(_paneId: PaneRecord["id"], input: string) {
    this.inputs.push(input);
  }
  public async resize(_paneId: PaneRecord["id"], cols: number, rows: number) {
...
const applicationCases = [
  {
    ...
    steps: [{ type: "list" }],
    assert: [
      hasNoError<ApplicationContext, ApplicationResult>(),
      returns<ApplicationContext, ApplicationResult>([pane]),
    ],
  },
  {
    ...
    assert: [
      hasNoError<ApplicationContext, ApplicationResult>(),
      returns<ApplicationContext, ApplicationResult>(undefined),
      hasObserved<ApplicationContext, ApplicationResult>("inputs", ["yes\n"]),
    ],
  },
  {
    ...
    assert: [
      hasNoError<ApplicationContext, ApplicationResult>(),
      returns<ApplicationContext, ApplicationResult>(undefined),
      hasObserved<ApplicationContext, ApplicationResult>("sizes", [[80, 24]]),
    ],
  },
  {
    ...
    assert: [
      hasError<ApplicationContext, ApplicationResult>({ message: "Pane not found: missing" }),
      hasObserved<ApplicationContext, ApplicationResult>("inputs", []),
    ],
  },
  {
...
> = {
  ...
  execute: async (fixture, steps) => {
    ...
    for (const step of steps) {
      ...
      if (step.type === "resize") {
        await new ResizePane(fixture.repository, fixture.gateway).execute(
          PaneId.create(step.paneId),
          step.cols,
          step.rows,
        );
        result = undefined;
  ...
  observe: (fixture) => ({
    inputs: [...fixture.gateway.inputs],
    sizes: fixture.gateway.sizes.map((size) => [...size] as [number, number]),
  }),
};
