...
export const createPane = Effect.fn("Panes.create")(function* (input: CreatePaneInput) {
  ...
  const recordWorkspaceId = workspaceId === undefined ? current.workspaceId : WorkspaceId.create(workspaceId);
  let record: MuximodPaneSummary = current.update({
    kind: input.kind,
    name: paneName,
  ...
  if (record.state !== desiredState) {
    record = record.transitionTo(desiredState, "pane created", clock.now());
  }
