  ...
  private runWorkflow(input: RunShellInput): Effect.Effect<RunShellResult, Error, never> {
    ...
    return Effect.gen(function* () {
      ...
      if (input.command.length > 0) {
        ...
        if (!executable) return yield* Effect.fail(new Error("shell command executable is missing"));
      ...
      const workspace = yield* deps.workspace.resolveCurrent();
      if (!workspace.isGit) return yield* Effect.fail(new Error("a managed worktree requires a git workspace"));
...
function releaseShellWorktree(
  ...
  return Effect.gen(function* () {
    ...
    if (!removal.success) return yield* Effect.fail(new Error("managed shell worktree was retained"));
    if (cleanup._tag === "Failure") return yield* Effect.fail(cleanup.failure);
    if (!cleanup.success) return yield* Effect.fail(new Error("cleanup hook failed"));
  });
}
