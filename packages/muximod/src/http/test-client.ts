import type { muximodContract } from "@muximo/contract/api";
...
export function createHttpTestClient(connection: TestConnection) {
  ...
  return {
    ...
    agentSessions: {
      run: (input: Parameters<RpcClient["agentSessions"]["run"]>[0]) => rpc.agentSessions.run(input),
      resume: (input: Parameters<RpcClient["agentSessions"]["resume"]>[0]) => rpc.agentSessions.resume(input),
    },
