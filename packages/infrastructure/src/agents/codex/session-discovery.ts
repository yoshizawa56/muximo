...
export type CodexSessionDeps = {
  ...
  audit(eventType: string, entityId: string, payload: unknown): ApplicationEffect<void>;
  manageRemoteThread(
...
export function collectCodexSessionBaseline(deps: CodexSessionDeps): { files: string[]; baseline: string } {
  const files = codexSessionFiles(deps);
...
export function discoverCodexSessionId(
  deps: CodexSessionDeps,
  startedAt: number,
  runDir: string,
  sessionId: string,
  endedAt?: number,
): ApplicationEffect<CodexDiscoveryResult> {
  return Effect.gen(function* () {
    const id = yield* attemptSync(() => AgentSessionId.create(sessionId));
    const session = yield* deps.sessions.findById(id);
    const state = yield* deps.state.find(id);
    ...
    const candidates = codexSessionCandidates(deps, codexSessionFiles(deps), startedAt, runDir, baseline, endedAt);
    const safeCandidates = yield* filterCodexSessionCandidates(
      deps,
...
export function filterCodexSessionCandidates(
  deps: CodexSessionDeps,
  candidates: CodexSessionCandidate[],
  diagnostics: Omit<CodexDiscoveryDiagnostics, "rootExists" | "elapsedMs">,
  session: AgentSession | undefined,
  runDir: string,
): ApplicationEffect<CodexSessionCandidate[]> {
  return Effect.gen(function* () {
    if (!session) return candidates;
    const sessions = yield* deps.sessions.list(session.workspaceId);
...
export function recoverCodexSessionId(
  ...
  return Effect.gen(function* () {
    const result = yield* discoverCodexSessionId(
      deps,
      0,
      runDir,
      session.id,
...
export function repairCodexSessionId(
  deps: CodexSessionDeps,
  session: AgentSession,
  runDir: string,
  phase: "run" | "resume",
): ApplicationEffect<AgentSession> {
  return Effect.gen(function* () {
    if (session.backend !== "codex" || session.backendSessionId) return session;
    const result = yield* recoverCodexSessionId(deps, session, runDir);
    if (!result.selectedId) {
      yield* deps.audit("agent_session.codex_session_id_recovery_failed", session.id, {
        name: session.name,
        phase,
        runDir,
        diagnostics: result.diagnostics,
    ...
    yield* deps.sessions.setBackendSessionIdIfMissing(session.id, result.selectedId);
    const persisted = yield* deps.sessions.findById(session.id);
...
export function reportCodexDiscoveryFailure(
  ...
  return Effect.gen(function* () {
    ...
    yield* deps.audit("agent_session.codex_session_id_missing", session.id, {
      name: session.name,
      phase,
      runDir,
      diagnostics: result.diagnostics,
...
function codexSessionFiles(deps: CodexSessionDeps): string[] {
  const root = codexSessionRoot(deps);
...
export function watchCodexSessionName(
  ...
  const run = async () => {
    while (!stopped) {
      const completed = await runEffectAsPromise(
        Effect.gen(function* () {
          const discovery = yield* discoverCodexSessionId(deps, startedAt, runDir, session.id);
          const selectedId = discovery.selectedId;
          if (!selectedId) return false;
          const update = yield* Effect.result(
            Effect.gen(function* () {
              yield* deps.sessions.setBackendSessionIdIfMissing(session.id, selectedId);
              const updated = yield* attemptSync(() => session.update({ backendSessionId: selectedId }));
              yield* fromPromise(() => deps.manageRemoteThread(updated, "name", controller.signal));
            }),
          );
